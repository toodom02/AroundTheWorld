...
class LinearSpline {
  ...
  AddPoint(t: number, d: any | THREE.Color) {
...
export class ParticleSystem {
  ...
  helperval: number;
  constructor(params: {scene: THREE.Scene; meteor: Meteor}) {
    const uniforms = {
      diffuseTexture: {
  ...
  _AddParticles(timeElapsed: number) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const size = (Math.random() * 0.5 + 0.5) * 4.0;
  ...
  _UpdateGeometry() {
    ...
    for (const p of this._particles) {
      positions.push(
        meteorPos.x + p.position.x - rad,
        meteorPos.y + p.position.y - rad,
        meteorPos.z + p.position.z - rad
      );
